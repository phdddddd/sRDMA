...
char const *remote_host = "192.168.1.20";
char const *nick_address = "192.168.1.10";
...
cxxopts::ParseResult parse(int argc, char *argv[]) {
  ...
  try {
    ...
    options.add_options()
      ...
      ("i,input", "Input", cxxopts::value<std::string>(), "FILE.bin")
      ...
      ("outstand", "Number of outstanding",cxxopts::value<uint32_t>()->default_value(std::to_string(128)), "N")
      ("sendsize", "sendsize in WQEs",cxxopts::value<uint32_t>()->default_value(std::to_string(256)),"N")
...
int main(int argc, char *argv[]) {
  ...
  std::vector<ReaderWriterQueue<request_t> *> qvec;
  ...
  for (uint32_t threadid = 0; threadid < numthreads; threadid++) {
    ...
    RequestInitWorker *w = new RequestInitWorker(
        threadid, qp, params.remote, params.rkey, maxoutstand, batch, recv_size,
        send_size, remregion);
    qvec.push_back(&(w->get_queue()));
    ...
    if (numthreads <= 4) {
      ...
      workers[attach_to_worker]->AddWorker(w);
    }
  }

  auto trace = new TraceReader(qvec, input, 0);
