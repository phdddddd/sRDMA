#include "worker/client_rw_bw.hpp"
...
cxxopts::ParseResult parse(int argc, char *argv[]) {
  ...
  try {
    ...
    options.add_options()
      ("readprob", "Read prob [0,100]",cxxopts::value<uint32_t>()->default_value(std::to_string(0)),"N")
...
int main(int argc, char *argv[]) {
  ...
  uint32_t readprob = allparams["readprob"].as<uint32_t>();
  ...
  */
  ...
  for (uint32_t threadid = 0; threadid < numthreads; threadid++) {
    ...
    if (numthreads <= 4) {
      Thread *t = new Thread(threadid, cb_timeout);
      t->install_worker(new ClientReadWriteWorker(
          threadid, readprob, qp, params.remote, params.rkey, N, maxoutstand,
          batch, len, recv_size, send_size, remregion));
      m->add_thread(t);
    } else {
      uint32_t attach_to_worker = threadid % 4;
      workers[attach_to_worker]->AddWorker(new ClientReadWriteWorker(
          threadid, readprob, qp, params.remote, params.rkey, N, maxoutstand,
          batch, len, recv_size, send_size, remregion));
    }
  }
