...
class ClientBandwidthWorker : public GenericWorker {
 public:
  ClientBandwidthWorker(uint8_t id, uint8_t type, SequreQP* qp,
...
void ClientBandwidthWorker::main_cb() {
  if (made_measurements < total_measurements || total_measurements == 0) {
    if (maxoutstand > outstanding && outstandingwqe < maxoutstand) {
      if (type == WRITE_TEST) {
        ...
        int ret;
        for (uint32_t req = 0; req < batch - 1; req++) {
          ret = qp->Read(wr_id, (uint64_t)receivemr->addr, receivemr->rkey, len,
                         remote_addr, remote_rkey, false, withremregion);
          //                  currentbuf = (currentbuf+1) % send_size;
          if (ret) info(log_fp, "Error\n");
        }
        ret = qp->Read(wr_id, (uint64_t)receivemr->addr, receivemr->rkey, len,
                       remote_addr, remote_rkey, true, withremregion);
        //                currentbuf = (currentbuf+1) % send_size;
        if (ret) info(log_fp, "Error sig\n");
      }
  ...
  if (qp->poll_recv_cq(&wc) > 0) {
    if (wc.status != IBV_WC_SUCCESS) info(log_fp, "recv cq error\n");
  ...
  if (qp->poll_send_cq(&wc) > 0) {
    outstandingwqe -= batch;
    if (wc.status != IBV_WC_SUCCESS) info(log_fp, "send cq error\n");
  }
}

void ClientBandwidthWorker::sometimes_cb() {
